...
class AddProfile(FlaskForm):
    ...
    gender = SelectField('Gender',choices=[('S','Select Gender'),('M','Male'), ('F','Female'), ('D','Decline to answer')],validators=[Required('Select a gender')])
    ...
    bio = StringField('Biography', validators=[Length(min=3, max=500),Required('Put something ya so,form wont validate when empty')])
    photo= FileField('Upload Profile Photo', validators=[
        FileRequired(),
